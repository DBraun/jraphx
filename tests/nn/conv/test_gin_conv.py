...
def test_gine_conv_bipartite():
    """Test GINE convolution with bipartite graphs."""
    key = random.key(42)
    x1 = random.normal(key, (4, 16))
    x2 = random.normal(random.key(123), (2, 16))
    # No separate edge_index needed for this test setup
    ...
    nn = MLP([16, 32, 32], rngs=nnx.Rngs(0))
    conv = GINEConv(nn, train_eps=True, rngs=nnx.Rngs(1))
    ...
    out = conv(x_combined, edge_index_adjusted, edge_attr)
    assert out.shape == (6, 32)
...
def test_gine_conv_eps_parameter():
    ...
    # Test with fixed eps
    nn1 = MLP([16, 32, 32], rngs=nnx.Rngs(0))
    conv1 = GINEConv(nn1, eps=0.5, train_eps=False)
    out1 = conv1(x, edge_index, edge_attr)
    assert out1.shape == (4, 32)

    # Test with trainable eps
    nn2 = MLP([16, 32, 32], rngs=nnx.Rngs(0))
    conv2 = GINEConv(nn2, eps=0.5, train_eps=True, rngs=nnx.Rngs(1))
...
def test_gine_conv_different_networks():
    ...
    # Test with simple MLP
    nn_simple = MLP([16, 32], rngs=nnx.Rngs(0))
    conv_simple = GINEConv(nn_simple, rngs=nnx.Rngs(1))
    out_simple = conv_simple(x, edge_index, edge_attr)
    assert out_simple.shape == (4, 32)

    # Test with deeper MLP
    nn_deep = MLP([16, 32, 64, 32], rngs=nnx.Rngs(0))
    conv_deep = GINEConv(nn_deep, rngs=nnx.Rngs(1))
...
def test_gine_conv_edge_attr_different():
    """Test that GINE returns differnet outputs with different edge_attr."""
    x = random.normal(random.key(42), (4, 16))
    edge_index = jnp.array([[0, 1, 2, 3], [0, 0, 1, 1]])
    ...
    # Test with edge attributes
    out_with_attr1 = conv(x, edge_index, edge_attr1)

    # Test without edge attributes
    out_with_attr2 = conv(x, edge_index, edge_attr2)

    # Should NOT be identical as different edge_attr should give different results
    assert not jnp.allclose(out_with_attr1, out_with_attr2)

def test_gine_conv_shapes():
    """Test GINE with different input shapes."""
    nn = MLP([10, 20], rngs=nnx.Rngs(0))
    conv = GINEConv(nn, rngs=nnx.Rngs(1))
    ...
    for num_nodes in [2, 5, 10]:
        ...
        if num_nodes > 1:
            ...
            edge_index = jnp.array([[0], [0]])  # Self-loop for single node
            edge_attr = jnp.array([[0]], dtype = jnp.float32)
        out = conv(x, edge_index, edge_attr)
        assert out.shape == (num_nodes, 20)


"""
test_gine_conv_empty_graph
Test GINE with empty edges.
I skipped this case as GINEConv on graphs with no edges is invalid
(PyG implementation raises error if edge_attr is invalid)
"""

def test_gine_conv_deterministic():
    """Test that GINE is deterministic with same inputs."""
    nn = MLP([6, 12], rngs=nnx.Rngs(42))
    conv = GINEConv(nn, rngs=nnx.Rngs(43))
...
def test_gine_conv_different_dtypes():
    ...
    # Test float64 - may be converted based on JAX settings
    x_f64 = jnp.ones((2, 3), dtype=jnp.float64)
    edge_attr_f64 = jnp.array([1, 2], dtype = jnp.float64).reshape(-1, 1)
    _ = conv(x_f64, edge_index, edge_attr_f64)  # Test that it works with float64
    # Note: output dtype depends on JAX default dtype settings
